...
var targetToggle = '#navbar-collapse-menu';
var isToggledClassName = 'expanded';
var navbarSelector = '.main-navbar';

var navbarToggler = {
    _isToggled: false,
};
Object.defineProperty(navbarToggler, 'isToggled', {
    get: function() {
...
function triggerToggle(isToggled) {
    var togglerButton = $(toggler);
    var togglerTarget = $(targetToggle);

    if (isToggled) {
        togglerButton.removeClass('collapsed');
        togglerTarget.addClass('show');
    } else {
        togglerButton.addClass('collapsed');
        togglerTarget.removeClass('show');
    }
}

function toggleButtonClickHandler() {
    navbarToggler.isToggled = !navbarToggler.isToggled;
}
...
function handleNavbarToggler() {
    $(toggler).on('click', toggleButtonClickHandler);

    $(document).click(function(event) {
        $target = $(event.target);
        if (!$target.closest(navbarSelector).length && $(navbarSelector).is(':visible')) {
            triggerToggle(false);
        }
    });
}
